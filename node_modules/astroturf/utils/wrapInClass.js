...
function wrapInClass(text) {
  const imports = [];
  let match;
  const rImports = /@import.*?(?:$|;)/g; // eslint-disable-next-line no-cond-assign

  while (match = rImports.exec(text)) {
    imports.push(match[0]);
  }

  text = text.replace(rImports, ''); // Components need two css classes, the actual style declarations and a hook class.
